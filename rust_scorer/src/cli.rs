...
//!
//! Issue #475: this module *is* the binary. `src/main.rs` is a thin shim that
//! calls [`main`], so the `rust_scorer` bin target links the library rather
//! than recompiling a second, independent copy of every module. One copy means
//! the shipped binary cannot drift from the code the benches and integration
//! tests exercise, and `dead_code` is armed across the whole crate.
