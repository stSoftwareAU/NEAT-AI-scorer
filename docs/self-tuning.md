...
| [#549](https://github.com/stSoftwareAU/NEAT-AI-scorer/issues/549) | reader-count-aware read defaults, the named aggregate budget, and removal of the dead `≥ 64 GiB → 256 MiB` read ceiling | [read-chunk retune](performance-baseline.md#read-chunk-retune-at-the-shipped-reader-count--10-august-2026-issue-549) | structure ships; the **chunk retune is a `negative-result`** — 32 MiB per reader was ~50 % slower, and no smaller size cleared this fleet's noise floor, so every tier keeps a byte-identical resident buffer |
