...
undetected. Closes #430.

The constant recorded production-corpus provenance (`training_data_files = 520`).
That figure is **already** documented in `docs/performance-baseline.md`, so
deleting the code loses no information — no doc move was required.

This mirrors sibling finding #429, which removed the adjacent
`PRODUCTION_CORPUS_BYTES` constant.
...
- `grep -rn "PRODUCTION_CORPUS_FILES" --include="*.rs" ...` now returns no
