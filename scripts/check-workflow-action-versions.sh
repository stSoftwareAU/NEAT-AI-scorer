...
# — its `uses:` references need the same SHA pinning as a workflow's. The
# directory is optional: the last local action (`setup-neat-core`) was retired
# when `neat-core` moved to a release-tag pin (Issue #630). An explicit
