...
#      refs containing a slash. A workflow that does not gate PRs at all has no
#      filter to check.
...
# `rustsec/audit-check` step counts. Both a duplicate and a total absence fail
# the gate: the first burns runner minutes on a verdict that can only agree,
# the second is a real hole in advisory coverage.
#
...
# Shared CLI harness (Issue #512) — the ok/fail reporting primitives and the
# not-found guards.
...
Options:
  ...
  --workflows DIR   Directory scanned for PR-time cargo audits (default: the
                    directory containing the workflow above).
...
# workflows with a `pull_request` trigger and follows local
# `uses: ./.github/workflows/<file>` calls to a fixed point, so an audit inside
# a reusable workflow is attributed to the PR event that reaches it.
pr_time_audit_workflows() {
  ...
  while [[ ${#frontier[@]} -gt 0 ]]; do
    workflow="${frontier[0]}"
    # Pop the head. Slicing an exhausted array is unsafe under `set -u` on
    # bash 3.2 (macOS), so the last element is dropped by assignment instead.
    if [[ ${#frontier[@]} -gt 1 ]]; then
      frontier=("${frontier[@]:1}")
    else
      frontier=()
    fi
    ...
    while IFS= read -r name; do
      callee="$dir/$name"
      # A plain `[[ … ]] && …` here would return non-zero on the last iteration
      # and, under `set -e`, abort this function's subshell mid-scan — a silent
      # short count. The `if` keeps the walk fail-loud instead.
      if [[ -f "$callee" ]]; then
        frontier+=("$callee")
      fi
...
PR_AUDITS=()
while IFS= read -r audit_workflow; do
  [[ -n "$audit_workflow" ]] && PR_AUDITS+=("$audit_workflow")
done < <(pr_time_audit_workflows "$WORKFLOWS_DIR")
...
case "${#PR_AUDITS[@]}" in
  ...
  0)
    ...
    ;;
  *)
    fail "PR-time cargo audit duplicated across ${#PR_AUDITS[@]} workflows — they read the same Cargo.lock against the same advisory database on the same pull_request event, so the second run can only repeat the first verdict at double the runner minutes (Issue #603): ${PR_AUDITS[*]}. Keep exactly one home: either the standalone workflow's pull_request trigger (its weekly cron stays either way) or the reusable workflow's audit step"
    ;;
esac
